...
globals [generation count-pickup count-dropoff sum-fitness count-collaboration]

;; turtles have reward for each actions that they take, finess value evaluated after each generation,
;; tick counter to hold robot during charing, and distances from package and drop-off zone
turtles-own [action-reward fitness distance-from-package distance-from-destination move]

;; 3 types of agents: robot, items, and charging station
breed [robot a-robot]
breed [package a-package]
;; charging station removed -assumption: solar panel charging system automatically
  ...
  repeat initial-num-robot [ ;; create robots
    ask one-of patches with [not any? turtles-here ;; do not place them over one another
      ...
      sprout-robot 1 [ ;; create a robot
        ...
        set move false ;; flag value for the collaborative robots to move together
...
to go
  ask package [ ;; package pick-up collaboration (2 robots are nearby 1 package)
    let near-robot robot-on neighbors ;; get the neighbor robot turtles nearby each package
    let collab-robot near-robot with [color = green] ;; get only hands-free turtles
    if count collab-robot >= 2 [ ;; 2 robots can collaborate
      let collab-1 one-of collab-robot ;; one of the agent from the agentset
      let collab-2 one-of collab-robot ;; temp agent
      while [collab-1 = collab-2] [ set collab-2 one-of collab-robot ] ;; get different one from collab-1
      ask collab-1 [ create-link-with collab-2 ] ;; collaboration relationship created
      ask collab-1 [
        set heading 270
        set color orange
        set action-reward action-reward + 15000 ;; reward for picking up an item & collaboration
      ]
      ask collab-2 [
        set heading 270
        set color orange
      ...
      set count-collaboration count-collaboration + 1 ;; number of collaboration count +1
      ask self [ die ] ;; pick-up package
    ]
  ]

  ask robot [ ;; manipulate robots' movement depending on charging status
    ifelse count link-neighbors = 1 [ ;; check self robot first
      if move = false [
        let action observation-to-action ;; put observation of each robot to cgp and solve the cgp
        if action = 1 [ fd 1 ] ;; speed boost up for carrying it together
        let wall wall-penalty
        let reward action-score-collaboration
        let drop drop-off
        ask link-neighbors [
          if action = 1 [ fd 2 ]
          if action = 2 [ lt 20 ]
          ...
          set action-reward action-reward + reward ;; sync reward with self
        ]
      ]
      ;; mark true so that it would not cause double-move
      ask self [ set move true ]
      ask link-neighbors [ set move true ]
    ]
    [
      let action observation-to-action ;; put observation of each robot to cgp and solve the cgp
  ...
  ask robot [ set move false ] ;; reset move flag so that it can move for the next tick
...
to-report observation-to-action
  let result-action 0 ;; return this to match the movement with the linked robot
  ...
  ifelse action-vector = (list 0 0 0) [ ;; if the action vector does not have an action to choose
    ...
    if action = 1 [
      fd 1 ;; move forward if it is 1
      set result-action 1
    ]
    if action = 2 [
      lt 20
      set result-action 2 ;; turn left if it is 2
    ]
    if action = 3 [
      rt 20 ;; turn right if it is 3
      set result-action 3
    ]
...
to-report drop-off
  let destination patch-here ;; check if it is on drop-off area
  if destination != nobody and pcolor = 118 and color = orange [ ;; if robot located in drop-off zone
    set action-reward action-reward + 30000 ;; reward for deliverying the package
    set color green ;; hands free now, so color it back to green
    set count-dropoff count-dropoff + 1 ;; number of drop-off count +1
    ask my-links [ die ] ;; remove the link
    report true
  ]
  report false
end

to-report action-score-collaboration
  ;; pick-up action is already rewarded in "go"
  let total-reward 0

  let current-distance-from-package sqrt((14 - xcor) ^ 2) ;; calculate the current distance between package and the robot
  let current-distance-from-destination sqrt((-16 - xcor) ^ 2) ;; calculate the current distance between destination area and the robot
  ifelse color = orange [ ;; if robot carrying package
    (ifelse current-distance-from-package < distance-from-package [ set total-reward total-reward +  150 ] ;; if approaching to drop-off zone, reward
      current-distance-from-package = distance-from-package [ set total-reward total-reward - 30 ] ;; if stagnated, penalize
    [ set total-reward total-reward - 50 ] ) ;; if moving further away from the drop-off zone, penalize
  ]
  [ ;; if robot without carrying package
    (ifelse current-distance-from-destination < distance-from-destination [ set total-reward total-reward + 150 ] ;; if approaching to pick-up area, reward
    current-distance-from-destination = distance-from-destination [ set total-reward total-reward - 30 ] ;; if stagnated, penalize
    [ set total-reward total-reward - 50 ]) ;; if moving further away from pick-up area, penalize
  ]
...
to-report get-in-cone [dist angle]
  ...
  [ set obs lput (7 - ((distance p) / 2)) obs ] ;; calculate the distance, offset it to make the range between 0
...
to reproduce
  if ticks mod tick-per-generation = 0 and ticks != 0 [ ;; in every num-generation tick
    ...
    ask robot [ ;; evalute fitness value of each turtle
      ...
      set fitness action-reward ;; calculate fitness value
    ...
    ask package [die]
    create-packages ;; re-create package
    ...
    set count-collaboration 0 ;; reset number of collaboration for the next generation
  ]
end

to-report reward-action
  ifelse ticks mod tick-per-generation = 0 [
    report (count-pickup + count-dropoff + count-collaboration) ;; reporter for the reward action
  ]
...
"num-collaboration" 1.0 0 -7500403 true "" "if ticks mod tick-per-generation = 0 and ticks != 0 [plot count-collaboration]"
